...
function set_default_emails_for_users()
{
	try {
		include_module("/../modules/mysql");
		$conn = get_mysqli_connection();
		if (!$conn) {
			basic_log_to_file("Failed to get MySQL connection for setting default emails");
			return false;
		}
		...
		$example_email = $_ENV['BREVO_EMAIL'] ?? getenv('BREVO_EMAIL') ?? null;
		if (!$example_email) {
			basic_log_to_file("BREVO_EMAIL not found in .env file");
			return false;
		}

		$email_parts = explode('@', $example_email);
		if (count($email_parts) !== 2) {
			basic_log_to_file("Invalid BREVO_EMAIL format in .env: $example_email");
			return false;
		}
		...
		basic_log_to_file("Using email pattern: {$email_prefix}+{username}@{$email_domain}");

		$sql1 = "UPDATE users 
				 SET email = CONCAT(?, '+', username, '@', ?) 
				 WHERE (email IS NULL OR email = '' OR email = '0') 
				 AND username IS NOT NULL 
				 AND username != '' 
				 AND username NOT LIKE '%@%'";

		$result1 = mysql_prepared_execute($sql1, [$email_prefix, $email_domain], $conn);
		$updated_rows1 = 0;

		if ($result1 && isset($result1['affected_rows'])) {
			$updated_rows1 = $result1['affected_rows'];
			basic_log_to_file("Updated $updated_rows1 users with default email addresses using username pattern");
		} else {
			basic_log_to_file("Error in query: " . ($GLOBALS["mysql_last_error"] ?? "Unknown error"));
			return false;
		}
		return true;
	} catch (Exception $e) {
		basic_log_to_file("Exception in set_default_emails_for_users: " . $e->getMessage());
		return false;
	}
}
$GLOBALS = [];
